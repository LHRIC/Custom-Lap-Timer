...
#define next_field()                            \
    ...
    while (*format)
    {
        ...
        { // NMEA talker identifier and type (union minmea_type *).
            ...
            memcpy(buf, field + 1,
                   (sizeof(*buf) - sizeof(buf->null_terminator)));
...
bool minmea_parse_gsa(struct minmea_sentence_gsa *frame, const char *sentence)
{
    ...
    if (!minmea_scan(
            sentence, "tciiiiiiiiiiiiifff", &frame->type, &frame->mode,
...
bool minmea_parse_gst(struct minmea_sentence_gst *frame, const char *sentence)
{
    ...
    if (!minmea_scan(
            sentence, "tTfffffff", &frame->type, &frame->time,
            &frame->rms_deviation, &frame->semi_major_deviation,
            &frame->semi_minor_deviation, &frame->semi_major_orientation,
            &frame->latitude_error_deviation, &frame->longitude_error_deviation,
                   ...
                   const struct minmea_time *time_)
{
    ...
    time_t timestamp =
        portable_timegm(&tm); /* See README.md if your system lacks timegm(). */
